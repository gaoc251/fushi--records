...
export default class UploadImage extends Component<propType, stateType> {
  ...
  onImageClick = (data) => {
    ...
    if (data.isAdd) {
      ...
      const count = Number(maxCount) - files.length + 1
      Taro.chooseImage({
        count: count, // 默认9
        // isSaveToAlbum: false,
        ...
        success: async function (res) {
          ...
          const fileBase64 = await that.fileToBase64(tmpFiles)
  
          that.setState({
            files: [...files, ...fileBase64]
          }, () => {
            that.props.onFileChange([...files, ...fileBase64])
          })
